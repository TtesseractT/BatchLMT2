...
def process_file(file_to_process, video_folder_name):
    try:
        ...
        subprocess.run(f'insanely-fast-whisper --model-name openai/whisper-large-v3 --file-name "{file_to_process}" --flash True --task transcribe --language en --device-id 0')
        # Run Processing
