...
def process_file(file_to_process, video_folder_name):
    try:
        ...
        subprocess.run(f'insanely-fast-whisper --file-name {file_to_process} --model-name openai/whisper-large-v3 --task transcribe --language en --device-id 0 --transcript-path {filenamestatic}.json')
